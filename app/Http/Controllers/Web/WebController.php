...
//        dd($sellers);
    ...
    public function getProductQuotation($id){
        // dd('getProductQuotation for : ' , $id , auth('customer')->user()->id);
        
        if(!auth('customer')->check()){
             Toastr::warning(translate('You must login first to can add quotation request'));
            return back();
        }
        
        $quotation  = Quotation::where("user_id",auth('customer')->user()->id)->where("product_id",$id)->first();
        if(!$quotation){
              Quotation::create([
        ...
        Toastr::success(translate('Your Quotation Send Successfully'));
        return back();
